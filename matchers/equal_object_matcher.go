...
// https://github.com/kubernetes-sigs/cluster-api/tree/main/internal/matchers
...
type EqualObjectMatcher struct {
	...
	options *EqualObjectMatcherOptions
}

// EqualObject returns a EqualObjectMatcher for the passed Kubernetes runtime.Object with the passed Options. This function can be
// used as a Gomega EqualObjectMatcher in Gomega Assertions.
func NewEqualObjectMatcher(original runtime.Object, opts ...EqualObjectMatchOption) *EqualObjectMatcher {
	matchOptions := &EqualObjectMatcherOptions{}
...
// the EqualObjectMatcher and EqualObjectMatcherOptions.
...
// calculateDiff applies the EqualObjectMatcherOptions and identifies the diff between the EqualObjectMatcher object and the actual object.
...
// MatchOption describes an Option that can be applied to a EqualObjectMatcher.
type EqualObjectMatchOption interface {
	// ApplyToEqualObjectEqualObjectMatcher applies this configuration to the given MatchOption.
	ApplyToEqualObjectMatcher(options *EqualObjectMatcherOptions)
}

// EqualObjectMatcherOptions holds the available types of EqualObjectMatcherOptions that can be applied to a EqualObjectMatcher.
type EqualObjectMatcherOptions struct {
	ignorePaths [][]string
	allowPaths  [][]string
}

// ApplyOptions adds the passed EqualObjectMatcherOptions to the EqualObjectMatcherOptions struct.
func (o *EqualObjectMatcherOptions) ApplyOptions(opts []EqualObjectMatchOption) *EqualObjectMatcherOptions {
...
// IgnorePaths instructs the EqualObjectMatcher to ignore given paths when computing a diff.
type IgnorePaths [][]string

// ApplyToEqualObjectMatcher applies this configuration to the given EqualObjectMatcherOptions.
func (i IgnorePaths) ApplyToEqualObjectMatcher(opts *EqualObjectMatcherOptions) {
	opts.ignorePaths = append(opts.ignorePaths, i...)
}

// AllowPaths instructs the EqualObjectMatcher to restrict its diff to the given paths. If empty the EqualObjectMatcher will look at all paths.
type AllowPaths [][]string

// ApplyToEqualObjectMatcher applies this configuration to the given EqualObjectMatcherOptions.
func (i AllowPaths) ApplyToEqualObjectMatcher(opts *EqualObjectMatcherOptions) {
...
// filterDiffMap limits the diffMap to those paths allowed by the EqualObjectMatcherOptions.
