...
var _ = Describe("WithJSONPathMatcher", func() {
	When("passed an invalid jsonpath", func() {
		It("should error", func() {
			matcher := NewHaveJSONPathMatcher("{^}", Succeed())

			success, err := matcher.Match(&corev1.Pod{})
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(MatchError(
				fmt.Errorf("JSON Path '{^}' is invalid: unrecognized character in action: U+005E '^'")))
	...
	When("passed an jsonpath to invalid location", func() {
		It("should error", func() {
			matcher := NewHaveJSONPathMatcher("{.blah}", Succeed())

			success, err := matcher.Match(&corev1.Pod{})
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(MatchError(
				fmt.Errorf("JSON Path '{.blah}' failed: blah is not found")))
			Expect(success).Should(BeFalse())
		})
	})
	...
	When("passed a valid jsonpath and empty object", func() {
		It("should succeed", func() {
			matcher := NewHaveJSONPathMatcher("{.metadata.name}", BeEmpty())
	...
	When("passed a valid jsonpath and unstructured object", func() {
		It("should succeed", func() {
			matcher := NewHaveJSONPathMatcher("{.metadata.name}", Equal("test"))
	...
	When("passed a valid jsonpath to a struct", func() {
		It("should return the struct", func() {
			matcher := NewHaveJSONPathMatcher("{.metadata}", MatchFields(IgnoreExtras, Fields{
				"Name":      Equal("my-pod"),
	...
	When("passed a valid jsonpath to an array", func() {
		It("should return multiple values", func() {
			matcher := NewHaveJSONPathMatcher("{.items[*].metadata.name}", HaveLen(3))
